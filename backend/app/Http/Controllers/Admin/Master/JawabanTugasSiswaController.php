...
class JawabanTugasSiswaController extends Controller
{
    ...
    public function index()
    {
        ...
        $jawabanTugasSiswas = Cache::remember('jawaban_tugas_siswa_list_' . $this->kdprofile(), 60, function () use ($perPage) {
            return App\Models\Master\JawabanTugasSiswa::with(['tugasSiswa', 'siswaTransaksi', 'guru','jawabanTugasSiswaDetails'])
    ...
    {
        $jawabanTugasSiswa = App\Models\Master\JawabanTugasSiswa::with(['tugasSiswa', 'siswaTransaksi', 'guru','jawabanTugasSiswaDetails'])
            ->where('statusenabled', $this->statusEnabled())
            ->where('kdprofile', $this->kdprofile())
            ->find($id);
    ...
    public function store(Request $request)
    {
        try {
            ...
            $jawabanTugasSiswa = App\Models\Master\JawabanTugasSiswa::create([
                ...
                'siswatransaksi_id' => $validatedData['siswatransaksi_id'],

...
const LoginCover = () => {
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!captcha) {
            showAlert(0, 'error', 'Please complete the ReCAPTCHA');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                // localStorage.setItem('authToken', meta.token);

                await setCookie('access_token', meta.token);
                await setCookie('role', `${data.roles}`);
                await setCookie('username', `${data.username}`);
                await setCookie('email', `${data.email}`);
                await setCookie('captcha', `${captcha}`);
                // API call successful, navigate to the next page
                showAlert(15, 'success', 'Signed in successfully');
                ...
                if (response.status === 401) {
                    ...
                    showAlert(401, 'error', 'Unauthorized access. Redirecting to login page');
                } else {
            ...
            <div className="relative flex min-h-screen items-center justify-center bg-[url(/assets/images/auth/map.png)] bg-cover bg-center bg-no-repeat px-6 py-10 dark:bg-[#060818] sm:px-16">
                ...
                <div className="relative flex w-full max-w-[1502px] flex-col justify-between overflow-hidden rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 lg:min-h-[758px] lg:flex-row lg:gap-10 xl:gap-0">
                    ...
                    <div className="relative flex w-full flex-col items-center justify-center gap-6 px-4 pb-16 pt-6 sm:px-6 lg:max-w-[667px]">
                        ...
                        <p className="absolute bottom-6 w-full text-center dark:text-white">© {new Date().getFullYear()}.TSRS All Rights Reserved.</p>
                    </div>
                </div>
            </div>
        </div>

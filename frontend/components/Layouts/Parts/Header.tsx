...
const Header = () => {
    ...
    const handleLogout = async () => {
        try {
            ...
            if (!token) {
                // Token tidak ada, mungkin pengguna belum login
                console.error('No authentication token found');
                showAlert(0, 'error', 'No authentication token found');
                return;
            }
            ...
            if (response.ok) {
                ...
                await deleteCookie('captcha');
                await deleteCookie('username');
                await deleteCookie('email');
                // Lakukan penanganan setelah berhasil logout
                showAlert(1, 'success', 'Logout success');
    ...
    const handleLockScreen = async () => {
        try {
            const role = getCookie('role')
            const token = getCookie('access_token')
            const tokusernameen = getCookie('username')
            if (!token) {
                // Token tidak ada, mungkin pengguna belum login
                console.error('No authentication token found');
                showAlert(0, 'error', 'No authentication token found');
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/logout/lock-screen`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
            ...
            if (response.ok) {
                await deleteCookie('access_token');
                // Lakukan penanganan setelah berhasil Lockscreen
                showAlert(1, 'success', 'Lockscreen success');
                // Contoh: Redirect ke halaman login
                await router.push('/lockscreen/');
            } else {
                // Handle errors as before
            }
        } catch (error) {
                        ...
                        <div className="dropdown flex shrink-0">
                            ...
                            >
                                <ul className="w-[230px] !py-0 font-semibold text-dark dark:text-white-dark dark:text-white-light/90">
                                    ...
                                    <li>
                                        <Link href="/lockscreen/" className="dark:hover:text-white"onClick={handleLockScreen}>
                                            <IconLockDots className="h-4.5 w-4.5 shrink-0 ltr:mr-2 rtl:ml-2" />
                                            Lock Screen
                                        </Link>
                                    </li>
                                    <li className="border-t border-white-light dark:border-white-light/10">
                                        <Link href="/auth/login" className="!py-3 text-danger" onClick={handleLogout}>
                ...
                <ul className="horizontal-menu hidden border-t border-[#ebedf2] bg-white px-6 py-1.5 font-semibold text-black rtl:space-x-reverse dark:border-[#191e3a] dark:bg-black dark:text-white-dark lg:space-x-1.5 xl:space-x-8">
                    ...
                    <li className="menu nav-item relative">
                        ...
                        <ul className="sub-menu">
                            ...
                            <li>
                                <Link href="https://tsrs.com" target="_blank">
                                    {t('documentation')}
                                </Link>
                            </li>
                        </ul>
